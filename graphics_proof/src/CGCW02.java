...
public class CGCW02{
	...
	class Renderer implements GLEventListener {
		...
		private float rotateXanglecone = -90;
		//Lighting parameter
		...
		public void display(GLAutoDrawable drawable) {
			...
			T.initialize();
			T.scale(0.3f, 0.3f, 0.3f);
			//move the cone downwards
			T.translate(0.0f, -0.0f, -0.5f);
			//without rotate, the cone is facing towards you, you would not be able to distinguish
			// if the cone was actually a cone or a circle.
			T.rotateX(rotateXanglecone);
			gl.glUniformMatrix4fv( ModelView, 1, true, T.getTransformv(), 0 );
			gl.glUniformMatrix4fv( NormalTransform, 1, true, T.getInvTransformTv(), 0 );
		...
		public void init(GLAutoDrawable drawable) {
			...
			//Create the second object (a cone) here, and set the materials

			SObject cone = new SCone(1,9,2, 2);
			idPoint=1;
			idBuffer=1;
			idElement=1;
			createObject(gl, cone);
			//different idPoint, idBuffer and idElement to allow different multiple shapes
			//to appear on the same screen.
			...
			Vec4 ambientProductCone = lightAmbient.times(materialAmbient2);
			ambient2 = ambientProductCone.getVector();
			Vec4 diffuseProductCone = lightDiffuse.times(materialDiffuse2);
			diffuse2 = diffuseProductCone.getVector();
			Vec4 specularProductCone = lightSpecular.times(materialSpecular2);
			specular2 = specularProductCone.getVector();
			//cone material different to the cone (Jade)
